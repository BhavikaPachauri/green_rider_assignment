...
const sanitizeStatus = (value) => {
    ...
    return VALID_STATUSES.includes(upper)
        ? upper
        : undefined;
};
...
const accessibleProjectIds = async (userId, isAdmin) => {
    if (isAdmin)
        return null;
...
const ensureProjectAccess = async (projectId, userId, isAdmin) => {
    if (isAdmin) {
...
export const getTaskById = async (req, res) => {
    ...
    if (req.userRole !== "ADMIN") {
        const access = await ensureProjectAccess(task.projectId, req.userId, false);
        if (!access) {
            res.status(403).json({ message: "Forbidden" });
            return;
        }
...
export const createTask = async (req, res) => {
    ...
    const access = await ensureProjectAccess(projectIdParsed, req.userId, req.userRole === "ADMIN");
    if (!access) {
        res
            .status(403)
...
export const updateTask = async (req, res) => {
    ...
    if (!isAdmin) {
        const access = await ensureProjectAccess(existing.projectId, req.userId, false);
        if (!access) {
            res.status(403).json({ message: "Forbidden" });
            return;
        }
